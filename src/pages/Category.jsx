...
const Category=()=> {
  ...
  if (loading) {
    return <><Spinner />{listings===null && <p className=" text-center text-3xl font-bold mt-6 mb-6">No Data Found</p>}</> ;
  }
  const onMoreData= async()=>{
    try{
      const listingRef=collection(db,"listings");
      const q = query(listingRef,where("type","==",params.categoryName),orderBy("timestamp","desc"),startAfter(lastFetchData),limit(4));
      const snapData= await getDocs(q);
      const lastVisiable=snapData.docs[snapData.docs.length - 1];
      setLastFetchData(lastVisiable);
      const listing=[];
  ...
  return (
    <div className="max-w-6xl mx-auto px-3">
      <h1 className=" text-center text-3xl font-bold mt-6 mb-6">{params.categoryName==="rent"?"Places for Rent":"Places for Sale"}</h1>
      {
